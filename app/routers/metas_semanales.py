...
def crear_meta_semanal(meta_semanal: MetaSemanalCreate, db: Session = Depends(get_db)):
    return crear_meta_semanal_service(meta_semanal,1,db)

@router.put("/{id}", response_model= MetaSemanalResponse)
def atualizar_meta_semanal(id: int, meta_semanal: MetaSemanalUpdate, db: Session = Depends(get_db)):
    return modificar_meta_semanal(id,db,meta_semanal)

@router.delete("/{id}")
def eliminar_meta_semanal(id: int, db: Session = Depends(get_db)):
